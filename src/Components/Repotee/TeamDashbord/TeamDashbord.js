...
const TeamDashbord = () => {
    ...
    const handelRequest=(task,status,totalHours,weekStart,id)=>{
                        ...
                        <div className="px-1 lg:px-0 overflow-auto scrollbar-hide h-32 lg:h-60">
                            {request.map((e, i) => (
                                <div key={i} className="flex justify-between gap-2 lg:gap-0 mb-2">
                                    <span className="w-1/5 py-1 text-xs text-center font-normal">Sohail Khan</span>
                                    ...
                                    <IoArrowForwardCircleOutline className="text-2xl text-center ml-3 lg:ml-6" onClick={()=>handelRequest(e.tasks,e.status,e.totalHours,e.weekRange.start,e.timesheetId)}/>
                                    </div>
                                </div>
                            ))}
                        </div>
